...
describe Chef::Resource::AzureNetworkInterface do
  ...
  it 'sets the location' do
    ...
    expect(my_vnet_resource.location('eastus2')).to eq('eastus2')
  end
  ...
  it 'raises an error when non-ip addresses are passed to private_ip_address' do
    my_vnet_resource = resource.new('interface')
    expect { my_vnet_resource.private_ip_address('test') }.to raise_error(Chef::Exceptions::ValidationFailed)
  end

  it 'raises an error when invalid ip addresses are passed to private_ip_address' do
    my_vnet_resource = resource.new('interface')
    expect { my_vnet_resource.private_ip_address('300.0.0.1') }.to raise_error(Chef::Exceptions::ValidationFailed)
  end
  ...
  it 'sets the private_ip_allocation_method to static' do
    my_vnet_resource = resource.new('interface')
    expect(my_vnet_resource.private_ip_allocation_method('static')).to eq('static')
  end

  it 'sets the private_ip_allocation_method to dynamic' do
    my_vnet_resource = resource.new('interface')
    expect(my_vnet_resource.private_ip_allocation_method('dynamic')).to eq('dynamic')
  end
